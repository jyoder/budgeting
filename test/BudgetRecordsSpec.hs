...
spec = do
  ...
  describe "Eq" $ do
    it "tests whether budget records are equal" $ do
      ...
      let teams = Teams.make ["T1", "T2"]
      let teammate = TeammateRecord.T 1 "Bhc" "Name" "Sports" teams teams teams teams
      let records1 = BudgetRecords.T [priority] [salary] [teammate]
      let records2 = BudgetRecords.T [priority] [salary] []
      records1 `shouldNotBe` records2
